...
WITH task_stats AS (
  ...
  LEFT JOIN LATERAL (
    SELECT t.id, t.completed, t.due_date
    FROM public.tasks t
    WHERE EXISTS (
      SELECT 1 FROM unnest(t.assigned_to) AS assignee(name)
      WHERE lower(btrim(assignee.name)) = lower(btrim(collaborator.name))
    )
  ) task ON TRUE
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE EXISTS (
      SELECT 1 FROM unnest(p.assigned_to) AS assignee(name)
      WHERE lower(btrim(assignee.name)) = lower(btrim(collaborator.name))
    )
  ) priority ON TRUE
  GROUP BY collaborator.id
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE EXISTS (
      SELECT 1 FROM unnest(d.assigned_to) AS assignee(name)
      WHERE lower(btrim(assignee.name)) = lower(btrim(collaborator.name))
    )
  ) deliverable ON TRUE
  GROUP BY collaborator.id
