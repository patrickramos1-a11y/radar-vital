...
import { Settings, LayoutDashboard, ClipboardList, Shield, FileText, Box, MessageSquare, Bell, Eye, List, Layers, ChevronDown, LucideIcon } from "lucide-react";
  ...
  return <Sidebar collapsible="icon" className="border-r border-sidebar-border">
      ...
      <SidebarHeader className="border-b border-sidebar-border">
        <div className="flex items-center gap-2 px-2 py-2">
          ...
          {!isCollapsed && <div className="overflow-hidden">
              <h1 className="text-base font-bold text-foreground tracking-tight leading-none truncate">Painel de Indicadores </h1>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {navigationItems.map(item => <SidebarMenuItem key={item.title}>
                  {'subItems' in item && item.subItems ? <CollapsibleNavItem item={item as NavItemWithSub} isCollapsed={isCollapsed} isActive={isSubMenuActive(item.subItems)} isActiveRoute={isActiveRoute} /> : (() => {
                const navItem = item as NavItemWithHref;
                const Icon = navItem.icon;
                return <SidebarMenuButton asChild isActive={isActiveRoute(navItem.href)} tooltip={navItem.title}>
      ...
      <SidebarFooter className="border-t border-sidebar-border">
        <div className="px-2 py-2">
          {!isCollapsed && <p className="text-[10px] text-muted-foreground text-center">
              © 2024 SISRAMOS
            </p>}
        </div>
      </SidebarFooter>
...
function CollapsibleNavItem({
  ...
  return <Collapsible defaultOpen={isActive} className="group/collapsible">
      <CollapsibleTrigger asChild>
        <SidebarMenuButton tooltip={item.title} isActive={isActive}>
          <Icon className="w-4 h-4" />
          <span>{item.title}</span>
          {!isCollapsed && <ChevronDown className="ml-auto w-4 h-4 transition-transform duration-200 group-data-[state=open]/collapsible:rotate-180" />}
        </SidebarMenuButton>
      </CollapsibleTrigger>
      <CollapsibleContent>
        <SidebarMenuSub>
          ...
          return <SidebarMenuSubItem key={subItem.href}>
