...
export function useAllClientsCommentCounts(currentUserName?: string): Map<string, number> {
  ...
  const fetchCounts = useCallback(async () => {
    try {
      ...
      const readField = currentUserName ? `read_${currentUserName.toLowerCase()}` as keyof typeof data[0] : null;
      const validFields = ['read_celine','read_gabi','read_darley','read_vanessa','read_patrick'];
      const useUserFilter = readField && validFields.includes(readField as string);
