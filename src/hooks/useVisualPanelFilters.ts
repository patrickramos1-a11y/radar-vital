...
import { VisualFilterFlags, VisualSortOption, SortDirection } from "@/components/visual-panels/VisualPanelFilters";

interface UseVisualPanelFiltersProps {
  clients: Client[];
...
export function useVisualPanelFilters({
  ...
  customSorter,
}: UseVisualPanelFiltersProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState<VisualSortOption>(defaultSort);
  const [sortDirection, setSortDirection] = useState<SortDirection>('desc');
  const [filterFlags, setFilterFlags] = useState<VisualFilterFlags>({
    priority: false,
    highlighted: false,
    withJackbox: false,
    checked: false,
  ...
  // Toggle filter flag
  const handleFilterFlagToggle = useCallback((flag: keyof VisualFilterFlags) => {
    setFilterFlags(prev => ({
      ...prev,
      [flag]: !prev[flag],
    }));
